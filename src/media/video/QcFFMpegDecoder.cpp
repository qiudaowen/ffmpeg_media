...
QcFFmpegDecoder::QcFFmpegDecoder(const AVCodecParameters *par, bool hw)
{
    Open(par, hw);
}

QcFFmpegDecoder::QcFFmpegDecoder(int srcW, int srcH, int srcFormat, int codecID, bool hw)
    : m_srcW(srcW)
    , m_srcH(srcH)
...
void QcFFmpegDecoder::Open(const AVCodecParameters *par, bool hw)
{
    do
    {
        Close();
...
void QcFFmpegDecoder::OpenCodec(const AVCodecParameters *par, AVCodec* pCodec)
{
...
int QcFFmpegDecoder::Decode(const char* dataIn, int dataSize, AVFrameRef& frame)
{
    AVPacket packet;
